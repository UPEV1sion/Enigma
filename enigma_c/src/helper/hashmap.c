...
static int32_t hm_resize(const HashMap hm)
{
...
int32_t hm_destroy(const HashMap hm)
{
    if (hm == NULL) return 1;
    free(hm->buckets);
    free(hm);
...
int32_t hm_set(const HashMap hm, const void *key, const void *value)
{
    size_t hash    = hm->hash_func(key, hm->key_size) % hm->capacity;
    Bucket *bucket = get_bucket(hm, hash);
...
int32_t hm_put(const HashMap hm, const void *key, const void *value)
{
...
int32_t hm_remove(const HashMap hm, const void *key)
{
